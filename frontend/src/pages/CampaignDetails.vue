...
const getProspectStatusClass = (status) => {
  ...
  return 'bg-theme-overlay text-theme-primary'
}
</script>
...
<template>
  <Layout>
    ...
    <div v-else-if="!campaign" class="text-center py-12">
      <h2 class="text-2xl font-bold text-theme-primary mb-2">Campaign Not Found</h2>
      <p class="text-theme-secondary mb-6">The campaign you're looking for doesn't exist.</p>
      <Button variant="primary" @click="router.push('/campaigns')">
        Back to Campaigns
      </Button>
    </div>
    ...
    <div v-else class="space-y-6">
      ...
      <div class="flex justify-between items-start">
        <div>
          <div class="flex items-center space-x-3 mb-2">
            <h1 class="text-2xl font-bold text-theme-primary">{{ campaign.name }}</h1>
            <span :class="['px-3 py-1 text-sm font-semibold rounded-full', getStatusBadgeClass(campaign.status)]">
              {{ capitalize(campaign.status) }}
            </span>
          </div>
          <p v-if="campaign.description" class="text-sm text-theme-secondary">{{ campaign.description }}</p>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="bg-theme-raised rounded-lg shadow p-6">
          <div class="text-sm text-theme-secondary mb-1">Total Prospects</div>
          <div class="text-3xl font-bold text-theme-primary">{{ campaign.total_prospects }}</div>
        </div>
        <div class="bg-theme-raised rounded-lg shadow p-6">
          <div class="text-sm text-theme-secondary mb-1">Processed</div>
          <div class="text-3xl font-bold text-linkedin">{{ campaign.processed_prospects }}</div>
        </div>
        <div class="bg-theme-raised rounded-lg shadow p-6">
          <div class="text-sm text-theme-secondary mb-1">Success</div>
          <div class="text-3xl font-bold text-green-600">{{ campaign.success_count }}</div>
        </div>
        <div class="bg-theme-raised rounded-lg shadow p-6">
          <div class="text-sm text-theme-secondary mb-1">Failed</div>
      ...
      <div class="bg-theme-raised rounded-lg shadow p-6">
        <div class="flex justify-between items-center mb-2">
          <h3 class="text-lg font-semibold text-theme-primary">Progress</h3>
          <span class="text-sm text-theme-secondary">{{ progressPercent }}% Complete</span>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-theme-raised rounded-lg shadow p-6">
          ...
          <dl class="space-y-3">
            ...
            <div>
              ...
              <dd class="text-sm text-theme-primary">{{ formatDate(campaign.created_at) }}</dd>
            </div>
            <div v-if="campaign.started_at">
              <dt class="text-sm font-medium text-theme-secondary">Started</dt>
              <dd class="text-sm text-theme-primary">{{ formatDate(campaign.started_at) }}</dd>
            </div>
            <div v-if="campaign.completed_at">
              <dt class="text-sm font-medium text-theme-secondary">Completed</dt>
              <dd class="text-sm text-theme-primary">{{ formatDate(campaign.completed_at) }}</dd>
            </div>
          </dl>
        </div>
        ...
        <div class="bg-theme-raised rounded-lg shadow p-6">
          <h3 class="text-lg font-semibold text-theme-primary mb-4">Campaign Actions</h3>
          <div v-if="campaign.steps && campaign.steps.length > 0" class="space-y-3">
            <div
              v-for="(step, index) in campaign.steps"
              :key="step.id"
              class="flex items-start space-x-3 p-3 bg-theme-overlay rounded-lg"
            >
              ...
              <div class="flex-1">
                ...
                <div v-if="step.message_template" class="text-xs text-theme-secondary mt-1">
                  Template: {{ step.message_template.name }}
                </div>
                <div v-if="step.delay_days > 0" class="text-xs text-theme-muted mt-1">
                  Delay: {{ step.delay_days }} day(s)
                </div>
              </div>
            </div>
          </div>
          <p v-else class="text-sm text-theme-muted">No actions configured</p>
        </div>
      </div>
      ...
      <div class="bg-theme-raised rounded-lg shadow">
        <div class="px-6 py-4 border-b border-theme">
          <h3 class="text-lg font-semibold text-theme-primary">Prospects in Campaign</h3>
        </div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-theme-overlay">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-theme-muted uppercase tracking-wider">Current Step</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-theme-muted uppercase tracking-wider">Last Action</th>
              </tr>
            </thead>
            <tbody class="bg-theme-raised divide-y divide-gray-200">
              <tr v-for="cp in campaign.campaign_prospects" :key="cp.id" class="hover:bg-theme-overlay">
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm font-medium text-theme-primary">
                    {{ cp.prospect?.full_name || 'Unknown' }}
                  </div>
                  <div class="text-sm text-theme-muted">{{ cp.prospect?.profile_url }}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-theme-muted">
        ...
        <div v-else class="px-6 py-12 text-center text-theme-muted">
          <p>No prospects in this campaign yet.</p>
        </div>
      </div>

      <!-- Danger Zone -->
      <div class="bg-theme-raised rounded-lg shadow p-6 border-2 border-red-200">
